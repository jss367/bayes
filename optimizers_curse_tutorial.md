# Optimizer's Curse: A Bayesian Shrinkage Tutorial

This tutorial demonstrates the **optimizer's curse**—a systematic bias that occurs when choosing among multiple alternatives based on noisy estimates. When you select the option with the highest estimated expected value, that estimate is more likely to be an overestimate than an underestimate. This creates "postdecision surprise" when the realized value falls short of expectations.

We'll show how Bayesian shrinkage can mitigate this curse. By pulling extreme estimates back toward a prior, we get more calibrated forecasts that reduce expected surprise and regret. Importantly, shrinkage can change which option we choose (by selecting based on posterior means rather than raw estimates), but even when the choice stays the same, the more calibrated forecast reduces postdecision disappointment. The tutorial covers two scenarios: zero-centered decisions and positive-shifted decisions.

## Setup and Imports

First, we import the necessary libraries. We'll use NumPy for simulations, Matplotlib for visualization, and Python's dataclasses to organize our configuration and results.
...
To organize our simulations, we define two dataclasses. `SimulationConfig` holds all the parameters for a simulation scenario, including the number of trials, options, and the statistical properties of true values, estimates, and outcomes. It also includes a method to compute Bayesian posterior means using the conjugate normal-normal model.

`SimulationResult` stores the outcomes from a simulation run, with properties that compute key metrics like postdecision surprise, optimism bias, and regret.
...
The heart of our simulation is `simulate_decisions`. For each trial, we generate true expected values for all options, then create noisy estimates (what the decision-maker sees) and noisy realized outcomes (what actually happens). We compute Bayesian posterior means by shrinking estimates toward the prior, then compare two decision rules: choosing the option with the highest estimate (naive) versus choosing the option with the highest posterior mean (Bayesian).

Note that Bayesian shrinkage can change which option is selected, since the posterior means differ from the raw estimates. However, the primary benefit is reducing regret and surprise: even when the same option is chosen, the more calibrated posterior mean forecast leads to less postdecision disappointment than the inflated raw estimate.

This function returns results for both methods so we can compare their performance.
...
## Sweeping Over Option Counts

To understand how the optimizer's curse intensifies with more alternatives, we run simulations across different numbers of options. This function sweeps over a range of option counts and aggregates the mean surprise and regret for each, allowing us to visualize how the curse grows.
...
### Postdecision Surprise Distribution

To see how Bayesian shrinkage reduces postdecision surprise, we plot histograms comparing the distribution of surprises (realized value minus forecast) for both methods. The vertical dashed lines show the mean surprise, making it easy to see which method has lower expected surprise.
...
This plot shows how well each method's forecasts align with the true values. Points on the diagonal line represent perfect calibration. The naive method tends to overestimate (points above the diagonal), while Bayesian shrinkage pulls estimates closer to the truth.
...
### Bias Curves Across Option Counts

This two-panel plot shows how surprise and regret change as we evaluate more alternatives. The left panel shows zero-centered scenarios, and the right shows positive-shifted scenarios. This visualization makes it clear that the curse intensifies with more options, and that Bayesian shrinkage provides consistent benefits.
...
This helper function formats simulation results for easy reading, showing mean surprise, optimism, and regret for each method.
...
Now we put it all together. We define two scenarios: a zero-centered scenario where true values cluster around zero, and a positive-shifted scenario where the average true value is above zero.

**Note on trial counts:** We use 20,000 trials here for statistical demonstration purposes—this is a Monte Carlo simulation to clearly show the average effects. In reality, you might only make 15 decisions in your lifetime, but we need many simulated trials to get stable estimates of mean surprise, regret, and other metrics. Each "trial" represents a hypothetical decision scenario, not an actual decision you'll make.

For each scenario, we run simulations, print summary statistics, and generate visualizations. Finally, we sweep over different numbers of options to show how the curse intensifies.
...
Running this script generates several figures in the `figures` directory:

- Histograms showing the distribution of postdecision surprise for both scenarios
- Scatter plots comparing estimates to true values for both scenarios
- A two-panel plot showing how surprise and regret change as the number of options increases

These visualizations demonstrate that the optimizer's curse becomes more severe with more alternatives, and that Bayesian shrinkage consistently reduces expected surprise and regret.
